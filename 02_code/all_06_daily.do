...
cap local dailyhfc_file "X:\Box Sync\Dissertation - Spotlight\01 Data\10_Survey Checks\spotlights_checks_${date_txt}"
cap local backcheck_file "X:\Box Sync\Dissertation - Spotlight\01 Data\10_Survey Checks\spotlights_backchecks_${date_txt}"

* Count total number of surveys collected
	preserve
		keep if  date_txt == "${date_txt}"
...
* Count number of surveys by village
	preserve
		...
		duplicates drop svy_village_n, force
		keep svy_village_n svy_ward_n svy_district_n svy_region_n svy_enum  count count_*
		sort svy_enum
